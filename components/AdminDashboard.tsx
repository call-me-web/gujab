...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ setPage, onNoticeUpdate }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const { data: currentNotice } = await supabase.from('announcements').select('id, message').eq('is_active', true).order('created_at', { ascending: false }).limit(1).maybeSingle();
            setActiveNoticeId(currentNotice?.id || null);
            setActiveNoticeMessage(currentNotice?.message || null);
    ...
    const handleDeactivateNotice = async () => {
        ...
        setPublishing(true);
        try {
            const { error } = await supabase.from('announcements').update({ is_active: false }).eq('id', activeNoticeId);
            if (error) throw error;
            onNoticeUpdate();
            fetchData();
        } catch (err: any) { alert(`Deactivation Failed: ${err.message}`); } finally { setPublishing(false); }
    };
    ...
    const masterLaunchSql = `-- GUJAB MASTER SQL v5.3 (Security Patch: Search Path)
...
-- SECURITY UPDATE: explicitly set search_path to public to prevent hijacking
            ...
            {showSqlHelp && (
                <div className="bg-gray-900 text-green-400 p-6 mb-8 border-4 border-black font-mono text-xs overflow-x-auto shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
                    <div className="flex justify-between items-center mb-4 border-b border-green-900 pb-2">
                        <span className="font-bold uppercase">DB REPAIR KIT (v5.3 - Security Patch)</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                        ...
                        {activeNoticeId && (
                            <div className="mt-6 pt-6 border-t font-secret">
                                <p className="text-[10px] uppercase text-gray-500 mb-2">Current Active Notice:</p>
                                <p className="text-sm bg-gray-50 p-2 border border-gray-200 mb-4 italic">"{activeNoticeMessage}"</p>
                                <button
                                    onClick={handleDeactivateNotice}
                                    disabled={publishing}
                                    className="w-full border-2 border-red-800 text-red-800 px-4 py-2 font-sans-condensed font-bold uppercase tracking-widest hover:bg-red-800 hover:text-white transition-all disabled:opacity-50"
                                >
                                    {publishing ? 'Deactivating...' : 'Kill Active Signal'}
                                </button>
                            </div>
                        )}
                    </div>
